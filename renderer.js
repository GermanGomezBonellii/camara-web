import {palettes} from './palettes.js?v=7';
export const categories={Colores:['Rojo profundo','Cianotipo','Terminal verde','CRT ámbar','Ultravioleta','Violeta lima','Acid','Aurora','Cromo rosa'],Básicos:['Saturación','Blanco y negro','Negativo','Blur','Pixelado'],Visión:['Infrarrojo','Mapa térmico','Visión nocturna','X-Ray'],Gráficos:['Solarizado','Poster rojo','Chrome','Pop rojo']};
export const effects=Object.values(categories).flat();
// Shader identifiers stay independent of menu order.
const shaderEffects=['Saturación','Blanco y negro','Negativo','Blur','Pixelado','Rojo profundo','Cianotipo','Terminal verde','CRT ámbar','Ultravioleta','Violeta lima','Acid','Infrarrojo','Mapa térmico','Visión nocturna','X-Ray','Solarizado','Poster rojo','Chrome','Pop rojo','Aurora','Cromo rosa'];
const paletteNames={'Rojo profundo':'LUT_ROJO_PROFUNDO','Infrarrojo':'LUT_INFRARROJO','Terminal verde':'LUT_TERMINAL_VERDE','Visión nocturna':'LUT_NIGHT_VISION','Cianotipo':'LUT_CIANOTIPO','CRT ámbar':'LUT_CRT_AMBAR','Ultravioleta':'LUT_ULTRAVIOLETA','Mapa térmico':'LUT_HEAT_MAP','X-Ray':'LUT_XRAY','Poster rojo':'LUT_POSTER_ROJO','Chrome':'LUT_CHROME','Acid':'LUT_ACID','Pop rojo':'LUT_POP_ROJO_ACID','Violeta lima':'LUT_VIOLETA_LIMA','Aurora':'LUT_AURORA','Cromo rosa':'LUT_CROMO_ROSA'};
...
if(effect==9){look=color(contrast(g,.35));look*=mod(floor(uv.y*size.y),3.)<1.?.96:1.;}
if(effect==21||effect==22)look=color(contrast(g,.25));
...
export class Renderer{
 constructor(){this.canvas=document.createElement('canvas');const gl=this.gl=this.canvas.getContext('webgl',{alpha:false,antialias:false,preserveDrawingBuffer:false});if(!gl)throw Error('Este navegador no tiene WebGL disponible. Probá con Chrome o Edge actualizado.');
